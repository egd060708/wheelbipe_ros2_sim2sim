...
protected:
  ...
  // Jump state publisher (用于在 RViz 中可视化跳跃数据)
  rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr jump_state_publisher_;
  ...
  double jump_active_time_ = 0.0;        // 跳跃已持续的累计时间（秒）
  bool jump_requested_ = false;          // 同步触发标志
  bool jump_enabled_ = false;
  double jump_duration_ = 0.4;      // 跳跃窗口持续时间（秒）
  double jump_height_value_ = 0.6; // 跳跃目标高度（用于策略输入）
  
  // 跳跃轨迹跟随相关
  double jump_initial_height_ = 0.0;  // 跳跃开始时的初始高度
  
  // 辅助函数：计算跳跃曲线值
  double computeJumpCurve(double progress) const;
